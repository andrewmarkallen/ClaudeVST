...
export class DeltaCache {
  private sessionCache: CacheEntry | null = null;
  private trackCaches = new Map<number, CacheEntry>();
  private deviceCaches = new Map<string, CacheEntry>();
   ...
   */
  resetCache(scope: 'all' | 'session' | 'tracks' | 'devices' = 'all'): void {
    switch (scope) {
      case 'all':
   ...
   */
  private computeDictDelta(oldDict: any, newDict: any, path = ''): Change[] {
   ...
   */
  private computeListDelta(oldList: any[], newList: any[], path: string): Change[] {
    const changes: Change[] = [];
   ...
   */
  private computeParameterDelta(oldState: any, newState: any): Change[] {
    const changes: Change[] = [];

    const oldParams = new Map<number, any>(
      (oldState.parameters || []).map((p: any) => [p.index, p])
    );
    const newParams = new Map<number, any>(
      (newState.parameters || []).map((p: any) => [p.index, p])
    );
    ...
    for (const [idx, newParam] of newParams.entries()) {
      ...
      if (oldParam) {
        ...
        if (oldParam.value !== (newParam as any).value) {
          changes.push({
            path: `parameters[${idx}].value`,
            type: 'modified',
            old_value: oldParam.value,
            new_value: (newParam as any).value,
          });
        }
      } else {

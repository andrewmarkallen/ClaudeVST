    ...
    this.server.setRequestHandler(ListToolsRequestSchema, async () => ({
      tools: [
        ...
        },
        {
          ...
          description:
            'Get session info with delta optimization (returns only changes since last call, 85-95% token savings)',
          inputSchema: { type: 'object', properties: {} },
        },
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
      try {
        ...
        if (name === 'get_track_delta') {
          ...
          const delta = this.cache.getTrackDelta((params as any).track_index as number, state);
          return {
            content: [{ type: 'text', text: JSON.stringify(delta, null, 2) }],
          };
        }
        ...
        if (name === 'get_device_delta') {
          ...
          const delta = this.cache.getDeviceDelta(
            (params as any).track_index as number,
            (params as any).device_index as number,
            state
          );
          return {
            content: [{ type: 'text', text: JSON.stringify(delta, null, 2) }],
        ...
        if (name === 'reset_delta_cache') {
          this.cache.resetCache((args?.scope as any) || 'all');
          return {
        ...
        if (name === 'list_device_presets') {
          const params = args as { device_type?: string };
        ...
        if (name === 'diagnose_sound_issue') {
          const params = args as { symptom: string; track_index?: number };
        ...
        const result = await this.ableton.sendCommand(name, args as any);

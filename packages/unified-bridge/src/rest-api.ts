...
export class RestAPI {
  ...
  private ableton: AbletonClient;
  private cache: DeltaCache;
  private port: number;

  constructor(port = 8080, ableton: AbletonClient, cache: DeltaCache) {
    this.port = port;
  ...
  private setupRoutes(): void {
    ...
    this.app.get<{ Params: { trackIndex: string } }>('/track/:trackIndex/info', async (request) => {
      try {
    ...
    this.app.get<{ Params: { trackIndex: string } }>('/track/:trackIndex/delta', async (request) => {
      try {
        const trackIndex = parseInt(request.params.trackIndex, 10);
        const state = await this.ableton.sendCommand('get_track_info', {
          track_index: trackIndex,
    ...
    this.app.get<{ Params: { trackIndex: string; deviceIndex: string } }>(
      '/track/:trackIndex/device/:deviceIndex/parameters',
      async (request) => {
        try {
          const trackIndex = parseInt(request.params.trackIndex, 10);
    ...
    this.app.get<{ Params: { trackIndex: string; deviceIndex: string } }>(
      '/track/:trackIndex/device/:deviceIndex/delta',
      async (request) => {
        try {
          const trackIndex = parseInt(request.params.trackIndex, 10);
          const deviceIndex = parseInt(request.params.deviceIndex, 10);
    ...
    this.app.post<{
      Params: { trackIndex: string; deviceIndex: string; paramIndex: string };
      Body: { value: number };
    }>(
      '/track/:trackIndex/device/:deviceIndex/parameter/:paramIndex',
      async (request) => {
        try {
          const trackIndex = parseInt(request.params.trackIndex, 10);
          const deviceIndex = parseInt(request.params.deviceIndex, 10);
    ...
    // Transport endpoints
    this.app.post<{ Body: { tempo: number } }>('/transport/tempo', async (request) => {
      try {
    ...
    // Clip endpoints
    this.app.post<{
      Params: { trackIndex: string; clipIndex: string };
    }>('/track/:trackIndex/clip/:clipIndex/fire', async (request) => {
      try {
        const trackIndex = parseInt(request.params.trackIndex, 10);
    ...
    this.app.post<{
      Params: { trackIndex: string; clipIndex: string };
    }>('/track/:trackIndex/clip/:clipIndex/stop', async (request) => {
      try {
        const trackIndex = parseInt(request.params.trackIndex, 10);
        const clipIndex = parseInt(request.params.clipIndex, 10);
    ...
    this.app.post<{
      Params: { trackIndex: string; clipIndex: string };
      Body: { length?: number };
    }>('/track/:trackIndex/clip/:clipIndex/create', async (request) => {
      try {
        const trackIndex = parseInt(request.params.trackIndex, 10);
        const clipIndex = parseInt(request.params.clipIndex, 10);
    ...
    this.app.post<{
      Params: { trackIndex: string; clipIndex: string };
      Body: { notes: any[] };
    }>('/track/:trackIndex/clip/:clipIndex/notes', async (request) => {
      try {
        const trackIndex = parseInt(request.params.trackIndex, 10);
        const clipIndex = parseInt(request.params.clipIndex, 10);
    ...
    // Track operations
    this.app.post<{ Body: { index?: number } }>('/track/create', async (request) => {
      try {
    ...
    this.app.post<{ Params: { trackIndex: string }; Body: { name: string } }>(
      '/track/:trackIndex/name',
      async (request) => {
        try {
          const trackIndex = parseInt(request.params.trackIndex, 10);
    ...
    // Browser endpoints
    this.app.get<{ Querystring: { category_type?: string } }>('/browser/tree', async (request) => {
      try {
    ...
    this.app.get<{ Querystring: { path: string } }>('/browser/items', async (request) => {
      try {
    ...
    this.app.post<{ Params: { trackIndex: string }; Body: { uri: string } }>(
      '/track/:trackIndex/load',
      async (request) => {
        try {
          const trackIndex = parseInt(request.params.trackIndex, 10);
    ...
    // Cache management
    this.app.post<{ Body: { scope?: string } }>('/cache/reset', async (request) => {
      try {
        const { scope = 'all' } = request.body;
        this.cache.resetCache(scope as any);
    ...
    this.app.get<{ Params: { deviceType: string; presetName: string } }>(
      '/presets/:deviceType/:presetName',
      async (request) => {
    ...
    this.app.post<{ Body: { command: string; args: Record<string, any> } }>('/command', async (request) => {
      try {
    ...
    this.app.post<{ Body: { audio_path: string } }>('/analyze', async (request) => {
      try {

...
export const DIAGNOSTIC_PATTERNS = [
    ...
    // High Frequency Issues
    {
        ...
        explanation: 'Hats should sit above everything else, crisp but not harsh. If theyre painful, theres usually a resonant peak in the 2-6 kHz range. Use a narrow Q to notch it out rather than broad high-frequency reduction.',
    },
    {
    ...
    // Dynamics Issues
    {
        ...
        explanation: 'For techno, you want 3-6 dB crest factor (peak minus RMS). If its lower, youre over-compressed. Check each compressor in the chain - combined gain reduction shouldnt exceed 6-8 dB total.',
    },
    // Stereo/Width Issues
    {
...
export function findMatchingPatterns(symptomText) {
    const lowerSymptom = symptomText.toLowerCase();
    return DIAGNOSTIC_PATTERNS.filter((pattern) => pattern.symptoms.some((s) => lowerSymptom.includes(s)));
}
